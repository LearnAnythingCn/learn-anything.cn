...
import MovieDetailComputer from "./movieDetailComputer"
...
const MovieDetailTemplate = ({ data }) => {

  var movie = data.strapiMovies;
  var title = movie.title, desc = movie.description, content = movie.content;

  var tags = [], keyId = 1;
  movie.movie_tags.forEach(
    element => {
  ...
  return (
    ...
    <div style={{ backgroundColor: '#f0f2f5' }}>
      ...
      {/* 手机客户端页面 */}
      <Mobile>
        <MovieDetailMobile toc={tocStr} md={mdStr} movie={movie} tags={tags} siteMetadata={data.site.siteMetadata} />
        {/* <ArticleMobile toc={tocStr} md={mdStr} node={data} author={author} tags={tags} /> */}
      </Mobile>

      {/* 非手机客户端页面 */}
      <Default>
        <MovieDetailComputer toc={tocStr} md={mdStr} movie={movie} tags={tags} siteMetadata={data.site.siteMetadata} />
      </Default>
...
export const queryMovie = graphql`
  query queryMoviePage($slug:String){
    strapiMovies(
      slug: {  eq: $slug }
    ){
        id
