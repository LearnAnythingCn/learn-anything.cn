...
const ArticleDetailComputer = ({ toc, md, node, author, tags }) => {

  var title = "";
  if (node.strapiArticles && node.strapiArticles.title) {
    title = node.strapiArticles.title;
  }

  var time = "";
  if (node.strapiArticles && node.strapiArticles.published_at) {
    const moment = require('moment')
    moment.locale('zh-cn');
    time = moment(node.strapiArticles.published_at).format('YYYY-MM-DD HH:mm:ss');
  }
...
export default ArticleDetailComputer
